...
- 테스트는 개발자의 개발 컴퓨터에서 Firebase Emualtor 를 실행해서 테스트하는 것을 원칙으로 한다.
  - 다만, 로컬 (개발자의) 컴퓨터에 에뮬레이터를 설치하고 관리 및 실행하는 것에 어려움이 있다면, 임시 (테스트 전용) 파이어베이스 프로젝트를 생성해서 실제 파이어베이스 프로젝트에 테스트를 실행 하는 것도 하나의 방법일 수도 있다.
...
- 테스트를 하기 위해서는 에뮬레이터를 실행하거나, 테스트 전용 파이어베이스 프로젝트에 테스트를 하는 것이 좋다. 실제 파이어베이스 프로젝트에 테스트하는 것은 권장하지 않는다.

- 본 프로젝트에서 테스트는 `mocha` & `chai` 패키지를 사용한다. 만약, mocha 와 chai 에 익숙하지 않다면, 공부를 조금 할 것을 추천한다.

- 테스트 명령은 `mocha` 명령어를 그대로 사용하면 되는데, 에뮬레이터에 접속하여 테스트를 하기 위해서는 파이어베이스 공식 문서에 설명되어져 있는 데로 EMULATOR HOST 를 지정해 주면 된다.

- 클라우드 함수 테스트 코드는 `functions/tests` 폴더에 기록 한다. 이 폴더에서 테스트를 하기 위해서는 `npm i` 를 하고 `npm test tests/**/*.spec.ts` 와 같이 전체 테스트 파일 실행 명령을 하면 된다.
  - 개별적인 테스트를 하기 위해서는 `package.json` 에 등록하고, `$ npm run test:user:crud` 와 같이 실행하면 된다.

- 참고로, 본인은 주로 `<project-root>/functions` 폴더에서 `t tests/test/firebase-auth.spec.ts` 와 같이 테스트를 실행한다. `t` 를 `npm run test` 명령으로 alias 를 통해 등록하고, 기타 설정을 한 것이다.
