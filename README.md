...
- 표준 코딩 스타일
  - 파이어베이스, Node.js, 유닛 테스트 등 가장 표준적인 코딩 스타일을 따르려고 노력하며, 가능한 자세한 주석을 달아서, 다른 개발자와 쉽게 소통을 할 수 있도록 한다.
...
- 본 프로젝트에서 테스트는 개발자의 개발 컴퓨터에서 Firebase Emualtor 를 실행해서 테스트하는 것을 원칙으로 한다.
  - 다만, 로컬 (개발자의) 컴퓨터에 에뮬레이터를 설치하고 관리 및 실행하는 것에 어려움이 있다면, 임시 (테스트 전용) 파이어베이스 프로젝트를 생성해서 실제 파이어베이스 프로젝트에 테스트를 실행 하는 것도 하나의 방법일 수도 있지만, 권하지는 않는다. 실제 사용하는 Firebase project 에서는 절대로 테스트를 하지 않아야 한다.

- 본 프로젝트에서 테스트는 Firebase Testing package 와 `mocha` & `chai` 패키지를 사용한다. 만약, mocha 와 chai 에 익숙하지 않다면, 공부를 조금 할 것을 추천한다.
  - 참고로, 테스트 명령은 `mocha` 명령어를 그대로 사용하면 되고, 에뮬레이터에 접속하여 테스트를 하기 위해서는 파이어베이스 공식 문서에 설명되어져 있는 데로 EMULATOR HOST 를 지정해 주면 된다. 이러한 표준적인 방법으로 테스트 작업을 해서 다른 개발자와 보다 쉽게 소통 할 수 있도록 하였다.

## 테스트 설치

- 파이어베이스 에뮬레이터를 설치한다. 공식 문서 참고.

- 클라우드 함수 테스트 코드는 `functions/tests` 폴더에 저장되어져 있다.
  - 이 폴더에서 테스트를 하기 위해서는 `npm i` 를 하고 `npm test tests/**/*.spec.ts` 와 같이 전체 테스트 파일 실행 명령을 하면 된다.
  - 개별적인 테스트를 하기 위해서는 `package.json` 에 등록하고, `$ npm run test:user:crud` 와 같이 실행하면 된다.

- 참고로, 본인은 주로 `<project-root>/functions` 폴더에서 `t tests/test/firebase-auth.spec.ts` 와 같이 테스트를 실행한다.
  - `t` 를 `npm run test` 명령으로 alias 를 통해 등록하고, 기타 설정을 한 것이다.
  - 가끔씩은 `<project-root>/functions` 폴더에서 `npm run test:test` 또는 `npm run test:user:create` 와 같이 테스트를 실행하기도 한다.
  - 자세한 내용은 `package.json` 파일을 참고하기 바란다.
...
- 클라우드 함수에 직접 연결해서 테스트하는 것으로, 클라우드 함수가 먼저 deploy 되어야 한다. 로컬 에뮬레이터에서 테스트를 한다면, 소스 코드 수정 후, 에뮬레이터를 종료하고, 다시 빌드해서, 에뮬레이터를 다시 실행해야 한다. 만약, 테스트 전용 파이어베이스 프로젝트에 테스트 한다면, 실제 deploy 해야 하는데, 시간이 오래 걸려 개발 작업의 흐름이 끊길 수 있다.
-  그래서, deploy 하기 전에 최대한 `코드 유닛 테스트`를 해야 한다.
...
- 먼저, 에뮬레이터를 실행한다. (만약, 에뮬레이터가 설치되지 않았다면 설치한다.)
  - 예) `<project> $ firebase emulators:start`

- 그리고 package.json 에 등록된 테스트를 실행한다.
  - 예) `$ npm run test:test`
  - 예) `<project/functions> $ t tests/test/test.spec.ts`
